...
const CustomerDashboard = ({
  ...
  // Определение мемоизированной функции fetchContests с помощью useCallback
  const fetchContests = useCallback(() => {
    // Очистка списка контестов перед загрузкой новых данных
    clearContestsList();
    getContests({
      limit: 8,
      offset: 0,
...
// Сопоставление состояния с свойствами компонента
const mapStateToProps = (state) => state.contestsList;
// Сопоставление действий с свойствами компонента
const mapDispatchToProps = (dispatch) => ({
  getContests: (data) =>
    dispatch(getContests({ requestData: data, role: CONSTANTS.CUSTOMER })),
  clearContestsList: () => dispatch(clearContestsList()),
